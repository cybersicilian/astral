...
export default class Server {
    ...
    addBot() {
        ...
        let id = Math.random().toString(36).substring(7);
        this.players[id] = new Player(this.serverConfig.startingHand, this.deck).setTurnPlacement(Object.keys(this.players).length).setName(Server.createName()).setBot();
    ...
    init(port = 15912) {
        ...
        this.serverObj = Bun.serve({
            ...
            websocket: {
                message(ws, message) {
                    server.log(`Message received: ${message}`);
                    ...
                    switch (result.type) {
                        ...
                        case CommEnum.GET_CHOICES:
                            ...
                            if (choiceCard.canBePlayed({
                                ...
                                let result = (JSON.stringify({
                                    ...
                                    card: choiceToGet,
                                    splits: choiceCard.getAbilities().map(ability => ability.getChoices({
                                        owner: server.players[id],
                                        opps: opps,
                                        deck: server.deck,
                                        card: choiceCard
                        ...
                        case CommEnum.PLAY_CARD:
                                ...
                                return selections.map((selection, choiceId) => {
                                    ...
                                    switch (type.choice) {
                                        ...
                                        case Choices.CARD_IN_HAND:
                                            return server.players[id].cih()[selection];
                            ...
                            if (server.activeTurn === id && server.turnPhase == 2 && card.canBePlayed({
                                ...
                                if (card.getChoices().length > 0) {
                                    server.gameLog(`${server.players[id].getLogText()} plays ${card.getLogText()} with the following choices:\n${choices.map((choice, index) => `${card.orderAbilities()[index].getFormulatedText({
                                        owner: server.players[id],
                                        opps: opps,
                                        deck: server.deck,
                                        card: card
                                    })}: ${choiceObjs[index].map(x => x.getLogText()).join(", ")}`).join("\n")}`);
                                }
                                else {
